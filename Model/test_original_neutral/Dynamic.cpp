...
//	f2.open("CheckRandomChoiceOfNeighbor.dat",ofstream::app);
	this->reorder_Sites();
	this->set_Pref();
...
//			sum=this->calc_SumN(ix_St1,ix_Sp1);//to calculate de sum of the [ P_ix_St1(s)-Pj(s) ]*w_ix_St1-j - NEUTRAL
			realMigration =0;
...
//			if (sum != 0 && this->_Sites.at(ix_St1).get_Nold(ix_Sp1) )//if there are inds of the species at site and there are preffered sites			
			if (this->_Sites.at(ix_St1).get_Nold(ix_Sp1) )//if there are inds of the species at site - NEUTRAL	
			{
				nNeigh = this->_Sites.at(ix_St1).get_NumberNeigh();
				vector<int> reorderedneigh;
				for (int auxk=0;auxk<nNeigh;auxk++){reorderedneigh.push_back(auxk);}
				random_shuffle(reorderedneigh.begin(), reorderedneigh.end());	
//				f2 << this->seed << " ";
//				f2 << ix_St1 << " ";
//				f2 << nNeigh << " ";
				for (k=0;k<nNeigh;k++)//number_mig is the number of migrations for species 'ix_Sp1', from site 'ix_St1' to site 'ix_TargetSt'.
				{
					int idneigh = reorderedneigh.at(k);
					auxNeigh = this->_Sites.at(ix_St1).get_NeighborhoodData(idneigh);
					ix_TargetSt = auxNeigh.id-1;			
//					f2 << ix_TargetSt << "(" << idneigh << ") ";
...
//					dif = ((this->_Sites.at(ix_TargetSt).get_Pref(ix_Sp1) - this->_Sites.at(ix_St1).get_Pref(ix_Sp1))*this->_Sites.at(ix_St1).get_NeighborhoodData(k).weight); - NEUTRAL
					
// 					if( mc_timestep==21 && ix_Sp1 == 2 && (ix_St1==0 || ix_TargetSt==0)) cerr << "ALE <MIG_1.1> dif:" << dif << endl;
					
//					if (dif > 0)//just will do the migration IF the Pref from the neighbohood be greater than the local Pref - NEUTRAL
//					{ // - NEUTRAL
...
//						number_mig = (int) (this->_Species.at(ix_Sp1).get_MigrationProbability() * (this->_Sites.at(ix_St1).get_Nold(ix_Sp1) - realMigration) *  dif/sum);//number of individuals that will migrate to this site! ALE					// - NEUTRAL	
						number_mig = (int) (this->_Sites.at(ix_St1).get_Nold(ix_Sp1) - realMigration) *  this->_Sites.at(ix_St1).get_Density(ix_Sp1);//number of individuals that will migrate to this site! ALE					// - NEUTRAL	
