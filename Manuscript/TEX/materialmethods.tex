...
respectively. $\mathcal{M}^{k_{\phi}}_{j}$ is the mobility probability
of species $k$ from site $j$, and it is defined as 1 -
$r^{k_{\phi}}_{j}$ where $r^{k_{\phi}}_{j}$ is the reproductive
success of species $k$ in site $j$. If individuals of species $k$ were
able to know their reproductive success in any site, then they would
have information to decide where to go to increase their reproductive
success. For example, if species $k$ reproduces in site $j$ better
than in any other site (high $r^{k_{\phi}}_{j}$), then individuals of
species $k$ prefer to stay in site $j$, thus the mobility probability
is close to zero. This describes the scenario of an informed
landscape. However, for non-informed landscapes, we assume individuals
of species $k$ do not have information about their reproductive
success in any site. Thus, the mobility probability is 1.

Mobility probability 1 

 This probability is 1 in the
non-informed model because reproductive success is not taken into
account to leave site $j$.
